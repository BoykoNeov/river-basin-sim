...
def test_rest_state_stays_at_rest():
    ...
    st = State.from_bed(bed, dx=10.0, depth=1.5, device=DEV)
    m0 = float(st.h.numpy().sum())
    for _ in range(20):
        step(st, dt=1.0, manning_n=N)
    h = st.h.numpy()
...
def test_flux_flows_downhill_in_x():
    ...
    st = State.from_bed(bed, dx=1.0, depth=depth, device=DEV)
    step(st, dt=0.01, manning_n=N)
    qx = st.qx.numpy()
...
def test_rainfall_adds_expected_volume():
    ...
    for _ in range(nsteps):
        step(st, dt=dt, manning_n=N, rain=rain)
...
def test_flux_limiter_conserves_mass_and_forbids_negative_depth():
    ...
    st = State.from_bed(bed, dx=10.0, depth=0.005, device=DEV)  # 5 mm sheet
    ledger = MassLedger.from_state(st)

    t = 0.0
    for _ in range(80):
        dt = compute_dt(st, alpha=0.7, dt_max=5.0)
        step(st, dt=dt, manning_n=0.03)  # limiter on by default
        t += dt
    ledger.record(st, t)
